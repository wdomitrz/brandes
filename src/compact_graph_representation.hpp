...
class Compact_graph_representation {
   ...
   private:
    void add_directed_edge(node_id node_1, node_id node_2,
                           std::vector<node_id>& considered_edges_from_node) {
        compact_graph[starting_positions_of_nodes[node_1] +
                      considered_edges_from_node[node_1]] = node_2;
        considered_edges_from_node[node_1]++;
    }
   ...
   public:
    Compact_graph_representation(
        ...
        for (const auto& [node_1, node_2] : edges)
            if (node_2 > max_node_id) max_node_id = node_2;
        ...
        for (const auto& [node_1, node_2] : edges) {
            starting_positions_of_nodes[node_1]++;
            starting_positions_of_nodes[node_2]++;
        }
        ...
        for (const auto& [node_1, node_2] : edges) {
            add_directed_edge(node_1, node_2, considered_edges_from_node);
            add_directed_edge(node_2, node_1, considered_edges_from_node);
        }
    }
